...
function sessionEntries(files, folder) {
...
function Overview() {
  ...
  const s=data.storage, state=s.free_percent<10||s.free_bytes<50e9?"danger":s.free_percent<20||s.free_bytes<100e9?"warn":"ok";
...
function Machine({machine}) {
  ...
  const zip=id=>`${api}/machines/${machine}/sessions/${encodeURIComponent(id)}/download`;
  const artifact=path=>`${api}/machines/${machine}/sessions/${encodeURIComponent(session.session_id)}/artifacts/${enc(path)}`;
  const tiles=sessionEntries(files,folder);
  const label=session?.display_name||session?.name||"Test";
  return <main><header className="topbar"><a href="/data/"><ArrowLeft size={15}/> MACHINES</a><b>{machine.toUpperCase()}</b><IconButton icon={dark?Sun:Moon} onClick={()=>setDark(!dark)}>{dark?"Light":"Dark"}</IconButton></header>
    ...
    {session&&!file&&<section className="browser card"><div className="file-heading"><div><IconButton icon={ArrowLeft} onClick={()=>setSession(null)}>All tests</IconButton><div className="breadcrumbs"><button onClick={()=>setFolder([])}>{label}</button>{folder.map((name,i)=><React.Fragment key={i}><ChevronRight size={14}/><button onClick={()=>setFolder(folder.slice(0,i+1))}>{name}</button></React.Fragment>)}</div></div><a className="button-link" href={zip(session.session_id)}><Download size={15}/>Download ZIP</a></div>{busy?<Spinner/>:<div className="tile-grid">{folder.length>0&&<button className="file-tile up" onClick={()=>setFolder(folder.slice(0,-1))}><ArrowLeft size={25}/><span>Up one folder</span></button>}{tiles.map(entry=>entry.kind==="folder"?<button className="file-tile folder" key={entry.name} onClick={()=>setFolder([...folder,entry.name])}><Folder size={30}/><span>{entry.name}</span></button>:<button className="file-tile" key={entry.file.path} onClick={()=>choose(entry.file)}>{entry.name==="session_manifest.json"?<FileJson size={30}/>:<FileText size={30}/>}<span>{entry.name}</span><small>{(entry.file.size_bytes/1024).toFixed(1)} KiB</small></button>)}</div>}</section>}
    {session&&file&&<section className="viewer card"><div className="file-heading"><div><IconButton icon={ArrowLeft} onClick={()=>{setFile(null);setPreview(null);setManifest(null)}}>Files</IconButton><div className="breadcrumbs"><button onClick={()=>setFile(null)}>{label}</button>{file.path.split("/").map((name,i)=><React.Fragment key={i}><ChevronRight size={14}/><span>{name}</span></React.Fragment>)}</div></div><a className="button-link" href={artifact(file.path)}><Download size={15}/>Download file</a></div>{busy?<Spinner label="Loading file"/>:manifest?<pre className="manifest">{manifest}</pre>:file.path.endsWith(".csv")?<CsvViewer preview={preview} page={offset=>loadCsv(file,offset)}/>:<div className="empty"><FileText size={42}/><h2>{file.name||file.path.split("/").at(-1)}</h2><p>This file type is not previewed in the portal.</p></div>}</section>}
  </main>;
}
